...
const Profile = () => {
  ...
  //COMMENT FOR TALHA:
  // is userid is equal to cookies.get("token").id, display all things
  // if not equal, then display only about page
  // alert(userid,cookies.get("token").id);
  ...
  useEffect(() => {
  const loggedUserId = cookies.get("token").id;
  ...
  return (
    <>
      ...
      <div class="mt-52 mb-20 bg-white rounded-xl shadow-lg p-10 w-2/3 mx-auto">
        ...
        {notOwner && <button onClick={(e) => { handleProfileSwitch(e) }} name="buying" class="notOwnerUser font-bold text-green-600 hover:text-green-700 block mx-auto">Switch to Selling</button>}

        <div class="flex justify-left mt-10">
          {/* put condition and hide/show buttons depending on the type of user from backend  */}
          <button name="profileInfo" id="btn-profileInfo" onClick={handleButton} class="tabsBtn w-42 border-2 border-green-600 text-green-600 font-semibold hover:text-white hover:bg-green-600 transition-colors  py-1 w-1/5 mx-2">Profile</button>
          {notOwner && <button name="bidsSec" id="btn-bidsSec" onClick={handleButton} class="tabsBtn w-42 switch-opt hidden border-2 border-green-600 text-green-600 font-semibold hover:text-white hover:bg-green-600 transition-colors  py-1 w-1/5 mx-2">Bids</button>}
          {notOwner && <button name="currOrdersSec" id="btn-currOrdersSec" onClick={handleButton} class="tabsBtn w-42 switch-opt hidden border-2 border-green-600 text-green-600 font-semibold hover:text-white hover:bg-green-600 transition-colors  py-1 w-1/5 mx-2">Current Orders</button>}
          {notOwner && <button name="offersIGotSec" id="btn-offersIGotSec" onClick={handleButton} class="tabsBtn w-42 switch-opt notOwnerUser border-2 border-green-600 text-green-600 font-semibold hover:text-white hover:bg-green-600 transition-colors  py-1 w-1/5 mx-2">Offers I Got</button>}
          {notOwner && <button name="myAdsSec" id="btn-myAdsSec" onClick={handleButton} class="tabsBtn w-42 switch-opt notOwnerUser border-2 border-green-600 text-green-600 font-semibold hover:text-white hover:bg-green-600 transition-colors  py-1 w-1/5 mx-2">My Ads</button>}
        </div>
