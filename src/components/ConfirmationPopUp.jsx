...
const ConfirmationPopUp = ({trigger,setTrigger,prevTrigger, ad, bid, description, title, fullname, skills, certification,message}) => {
  ...
  const makebid = (ad_id, bid, description, title) => {
    const user_id = cookies.get("token").id;
    console.log("makebid called hi", ad_id)
  ...
  const performAction = () => {
    if (prevTrigger === true) {
      ...
      console.log(ad.id)
      makebid(ad.id, bid, description, title)
      setCongratulationsPopUpBtn(true)
    }
  ...
  <div className='bg-black absolute top-0 bg-opacity-50 z-20 w-full h-[800px]'>
        ...
        <h1 class="text-xl text-center font-extrabold my-10">Please confirm your request!</h1>
